...
export default function ClaimDetails() {
  ...
  if (claimData?.preauth_form) {
    const f = claimData.preauth_form;
    const patient = f.patient || {};
    const proc = f.procedure || {};
    const summary = f.evidenceSummary || {};
    const docs = Array.isArray(f.requiredDocuments) ? f.requiredDocuments : [];
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          <header style={{
            ...
            <div>
              ...
              <p style={{ margin: '0.4rem 0 0', color: 'var(--text-muted)', fontSize: '0.95rem' }}>
                Dynamic result generated from your form input.
              </p>
            </div>
            <div style={{
              background: 'var(--bg-card)', padding: '1rem 1.25rem', borderRadius: 'var(--radius-md)',
              border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-sm)', textAlign: 'right',
            }}>
              <p style={{ margin: '0 0 0.2rem', fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                Claim ID: <span style={{ fontWeight: 700, color: 'var(--text-primary)', fontFamily: 'JetBrains Mono, monospace' }}>{id}</span>
              </p>
              <p style={{ margin: 0, fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                Generated: <span style={{ fontWeight: 700, color: 'var(--text-primary)' }}>{new Date(f.generatedAt || Date.now()).toLocaleString()}</span>
              </p>
            </div>
          </header>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem', alignItems: 'start' }}>
            <div style={{
              ...
              <h3 style={{ margin: '0 0 1rem', fontSize: '0.85rem', fontWeight: 800, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.6px' }}>
                Patient & Procedure
              </h3>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.9rem' }}>
                <div>
                  <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase' }}>Patient</div>
                  <div style={{ fontSize: '1rem', color: 'var(--text-primary)', fontWeight: 800 }}>{patient.patientName || '—'}</div>
                  <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: 2 }}>
                    Policy: <span style={{ fontFamily: 'JetBrains Mono, monospace', fontWeight: 800, color: 'var(--text-secondary)' }}>{patient.policyNumber || '—'}</span>
                  </div>
                </div>
                <div>
                  <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase' }}>Insurer</div>
                  <div style={{ fontSize: '1rem', color: 'var(--text-primary)', fontWeight: 800 }}>{patient.insurerId || '—'}</div>
                  <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: 2 }}>
                    Patient ID: <span style={{ fontFamily: 'JetBrains Mono, monospace', fontWeight: 800, color: 'var(--text-secondary)' }}>{patient.patientId || '—'}</span>
                  </div>
                </div>
                <div style={{ gridColumn: '1 / -1' }}>
                  <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase' }}>Procedure</div>
                  <div style={{ fontSize: '1rem', color: 'var(--text-primary)', fontWeight: 800 }}>{proc.procedureName || '—'}</div>
                  <div style={{ display: 'flex', gap: 10, marginTop: 6, flexWrap: 'wrap' }}>
                    <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.82rem', fontWeight: 800, color: 'var(--text-secondary)', background: 'var(--bg-secondary)', border: '1px solid var(--border-color)', padding: '4px 8px', borderRadius: 8 }}>
                      {proc.cptCode || '—'}
                    </span>
                    <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.82rem', fontWeight: 800, color: 'var(--text-secondary)', background: 'var(--bg-secondary)', border: '1px solid var(--border-color)', padding: '4px 8px', borderRadius: 8 }}>
                      ICD-10 {proc.icd10Code || '—'}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div style={{
              ...
              boxShadow: '0 20px 40px rgba(0,0,0,0.35)', overflow: 'hidden',
              ...
              <div style={{
                ...
                <h3 style={{ margin: 0, color: '#fff', fontSize: '1rem', fontWeight: 800, letterSpacing: '0.5px' }}>AI Evidence (Dynamic)</h3>
                <span style={{ fontSize: '0.75rem', fontWeight: 800, color: 'var(--med-red)' }}>
                  Risk: {String(summary.riskLevel || '—')} ({summary.riskScore ?? '—'})
                </span>
              </div>
              <div style={{ padding: '1.25rem' }}>
                <div style={{ display: 'flex', gap: 10, marginBottom: '1rem', flexWrap: 'wrap' }}>
                  <span style={{ color: 'rgba(255,255,255,0.7)', fontSize: '0.8rem', fontWeight: 700 }}>
                    Provided: {summary.documentsProvided ?? 0}
                  </span>
                  <span style={{ color: 'rgba(255,255,255,0.7)', fontSize: '0.8rem', fontWeight: 700 }}>
                    Missing: {summary.documentsMissing ?? 0}
                  </span>
                  <span style={{ color: f.submissionReady ? '#4ade80' : '#fca5a5', fontSize: '0.8rem', fontWeight: 800 }}>
                    {f.submissionReady ? 'Submission Ready' : 'Needs Documents'}
                  </span>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {docs.map((d, idx) => (
                    <div
                      key={idx}
                      onMouseEnter={() => setActiveHighlight(String(idx))}
                      onMouseLeave={() => setActiveHighlight(null)}
                      style={{
                        padding: '0.9rem 1rem',
                        borderRadius: 12,
                        border: '1px solid rgba(255,255,255,0.08)',
                        background: activeHighlight === String(idx) ? 'rgba(255,255,255,0.06)' : 'rgba(255,255,255,0.02)',
                        cursor: 'default',
                        transition: 'all 0.2s ease',
                      }}
                    >
                      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10 }}>
                        <div style={{ color: '#fff', fontWeight: 800, fontSize: '0.9rem' }}>{d.document}</div>
                        <div style={{ color: d.status === 'provided' ? '#4ade80' : '#fca5a5', fontWeight: 900, fontSize: '0.78rem', textTransform: 'uppercase' }}>
                          {d.status}
                        </div>
                      </div>
                      <div style={{ marginTop: 6, color: 'rgba(255,255,255,0.7)', fontSize: '0.82rem', lineHeight: 1.35 }}>
                        {d.citation || 'No citation available'}
                      </div>
                    </div>
                  ))}
                </div>

                {Array.isArray(summary.topDrivers) && summary.topDrivers.length > 0 && (
                  <div style={{ marginTop: '1rem', color: 'rgba(255,255,255,0.75)', fontSize: '0.82rem' }}>
                    <div style={{ fontWeight: 900, color: '#fff', marginBottom: 6 }}>Top drivers</div>
                    <ul style={{ margin: 0, paddingLeft: '1.1rem' }}>
                      {summary.topDrivers.map((t, i) => <li key={i}>{t}</li>)}
                    </ul>
                  </div>
                )}
              </div>
